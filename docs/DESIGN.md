...
**Why JzCzhz over OKLCH?** OKLCH (Ottosson 2020) is intrinsically more uniform — its hue steps vary less across the wheel. But this theme ships in sRGB, and every design color must survive gamut clipping. In practice, JzCzhz held up better for this palette once clipped back into sRGB, especially on the warm hues that carry error, warning, function, and class tokens. That tradeoff matters more here than absolute hue-step uniformity because hue assignments are semantic (each hue has a fixed role), not comparative (users do not judge neighboring steps as a continuous scale). If VS Code ever supports wide-gamut output, OKLCH should be reconsidered.
