...
export function createTerminalTokens(p: Primitives): TerminalTokens {
  ...
  return {
    ...
    red: role('Alert -- rose, above mezzo for Lc>=60 on warm hue', L.mezzo + 0.010, C.mf, H.tritone),
    green: role('Success -- green 150 (blue-axis for deutan safety), just under soprano for deutan DJz vs yellow', L.soprano - 0.005, C.f, H.major7th),
    ...
    white: role('Muted foreground -- warm off-white, low chroma; under soprano for deutan DJz vs cyan', L.soprano - 0.003, C.pp, H.peach),
    brightBlack: role('Comment gray -- below soprano, dark gray', L.alto, C.ppp, H.minor2nd),
    brightRed: role('Bright alert -- rose, above soprano for Lc>=60 on warm hue', L.soprano + 0.007, C.f, H.tritone),
