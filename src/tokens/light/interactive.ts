...
export function createInteractiveTokens(
  ...
  const btnHover = hex({ Jz: cushion.Jz - 0.015, Cz: cushion.Cz + 0.015, hz: cushion.hz });   // Slightly darker pink on hover
  const btnActive = hex({ Jz: cushion.Jz - 0.025, Cz: cushion.Cz + 0.020, hz: cushion.hz });  // Deeper pink on press
