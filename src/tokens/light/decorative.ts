/**
 * Light Decorative Token Definitions
 *
 * The hidden love letters in the theme. Indent guides trace warm pastry
 * colors. SCM graph uses Project SEKAI unit colors darkened for snow.
 * Chart colors come from Magical Mirai concerts. These details are
 * invisible unless you look for them — which is exactly right.
 */
...
export function createSymbolTokens(p: Primitives): SymbolTokens {
  ...
  return {
    ...
    snippet: role('Symbol snippet -- magenta, quiet template', L.soprano + 0.006, C.p, H.magenta),
  };
}
...
export function createDecorativeTokens(p: Primitives): DecorativeTokens {
  return {
    // Indent guides -- warm pastry colors for the patisserie theme
    indentGuides: [
    ...
    diffInserted: hex({ Jz: 0.120, Cz: 0.110, hz: 150 }),
    diffRemoved: hex({ Jz: 0.120, Cz: 0.110, hz: 15 }),
    ...
    inlayParameter: hex({ Jz: 0.095, Cz: 0.020, hz: 50 }),
    statusItemForeground: character.headphones.frame,
    markupInserted: hex({ Jz: 0.075, Cz: 0.080, hz: 150 }),
    ...
    walletChain: p.character.skirt.accessory, // Variant-aware -- warm gray on cream
    ...
    cursorLineFrost: hex({ Jz: 0.130, Cz: 0.060, hz: 220 }), // Darker ice blue for visibility on snow
