...
export const WEB_THEME_ARTIFACTS = {
  ...
  token: (slug: string) => `${WEB_THEME_BASENAME}-${slug}.tokens.json`,
  tokens: {
...
/** The registry-owned fields needed by the shared Web artifacts. */
...
function webFlagship(allThemes: readonly WebTheme[], polarity: Polarity): WebTheme {
  const matches = allThemes.filter(theme => theme.flagship && theme.polarity === polarity);
  if (matches.length !== 1 || matches[0].slug !== polarity) {
    throw new Error(`Web CSS requires exactly one '${polarity}' flagship with the '${polarity}' slug`);
  }
  return matches[0];
}

function renderExplicitTheme(theme: WebTheme): string {
  return `[data-hm-theme='${theme.slug}'] {
  color-scheme: ${theme.polarity};
${renderCssDeclarations(theme.tokens, theme.polarity)}
}`;
}

export function createWebCss(allThemes: readonly WebTheme[]): string {
  const light = webFlagship(allThemes, 'light');
  const dark = webFlagship(allThemes, 'dark');
  const lightDeclarations = renderCssDeclarations(light.tokens, light.polarity);
  const darkDeclarations = renderCssDeclarations(dark.tokens, dark.polarity);
  const nestedDarkDeclarations = darkDeclarations.replace(/^/gm, '  ');
  const additionalThemes = allThemes
    .filter(theme => !theme.flagship)
    .map(renderExplicitTheme)
    .join('\n\n');
  return `/**
